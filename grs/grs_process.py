...
from . import AuxData, CamsProduct, L2aProduct, Masking, Product, Rasterization, acutils
...
class Process:
    ...
    def execute(
        ...
        if surfwater_file:
            ...
            prod.raster["surfwater"] = (
                rio.open_rasterio(surfwater_file)
                .astype(np.uint8)
                .squeeze()
                .interp(x=prod.x, y=prod.y, method="nearest")
            )
        ...
        for wl in prod.raster.wl.values:
            prod.raster["bands"].loc[wl] = prod.raster.bands.sel(wl=wl) / Tg_raster.sel(
                wl=wl
        ...
        prod.raster.ndwi.attrs = {
            "description": "Normalized difference spectral index between bands at "
            + str(prod.bvis)
            + " and "
            + str(prod.bnir)
            + " nm",
        ...
        if allpixels:
            ...
            mask = (ndwi_swir > prod.vis_swir_index_threshold) & (
                swir2 < prod.sunglint_threshold
            )  # (ndwi > -0.0) &
        ...
        if opac_model is None:
            lut_aod = aero_lut.aot.sel(model=models, aot_ref=1).interp(
                wl=cams.cams_aod.wl
            )
            idx = (
                np.abs((cams_aot_mean / cams_aot_ref_mean) - lut_aod).sum("wl").argmin()
            )
        ...
        szamin, szamax = (
            float(prod.raster["sza"].min()),
            float(prod.raster["sza"].max()),
        )
        vzamin, vzamax = (
            float(prod.raster.isel(wl=0)["vza"].min()),
            float(prod.raster.isel(wl=0)["vza"].max()),
        )
        ...
        azi_ = (
            180 - np.unique(prod.raster.isel(wl=0)["raa"].round(ang_resol["raa_round"]))
        ) % 360
        ...
        aot_ref_min = 0.0  # aot_ref_raster.min()
        aot_ref_max = aot_ref_raster.max()
        aot_lut = aero_lut_.aot.interp(wl=wl_true, method="quadratic")
        aot_lut = aot_lut.interp(
            aot_ref=np.linspace(aot_ref_min, aot_ref_max.values, 1000)
        )  # .plot(hue='wl')
        aaot_lut = aero_lut_.aaot.interp(wl=wl_true, method="quadratic")
        aaot_lut = aaot_lut.interp(
            aot_ref=np.linspace(aot_ref_min, aot_ref_max.values, 1000)
        )

        Rdiff_lut = (
            aero_lut_.I.sel(sza=sza_slice, vza=vza_slice)
            .interp(wl=wl_true, method="quadratic")
            .interp(azi=azi_)
        )
        Rdiff_lut = Rdiff_lut.interp(sza=sza_, vza=vza_)
        Rray = Rdiff_lut.sel(aot_ref=0)
        Rdiff_lut = Rdiff_lut.interp(
            aot_ref=[0, 0.02, 0.05, 0.07, *aot_ref_], method="quadratic"
        )  # .sortby("aot_ref")
        ...
        Ttot_Ed_ = (
            Ttot_Ed.sel(model=opac_model)
            .sel(wind=_wind, method="nearest")
            .interp(sza=szas)
            .interp(aot_ref=aot_ref_, method="quadratic")
            .interp(wl=wl_true, method="cubic")
            .Ttot_Ed
        )
        Ttot_Lu_ = (
            Ttot_Ed.sel(model=opac_model)
        ...
        aot_ref_raster = (
            aot_ref_raster.interp(x=prod.raster.x, y=prod.raster.y)
            .drop("wl")
            .astype(np.float32)
        )
        ...
        if dem_file:
            ...
            dem = (
                xr.open_dataset(dem_file)
                .squeeze()
                .interp(y=prod.raster.y, x=prod.raster.x, method="nearest")
            )
            ...
            _pressure = (
                presure_msl * (1.0 - 0.0065 * dem.dem / 288.15) ** 5.255
            ).values
        else:
            dem = None
        ...
        Tg_diff_raster = gas_trans.get_gaseous_transmittance(
            gases=gases, background=False
        ).transpose("wl", "y", "x")
        ...
        Rrs_result = np.ctypeslib.as_ctypes(
            np.full((Nwl, height, width), np.nan, dtype=prod._type)
        )
        Rf_result = np.ctypeslib.as_ctypes(
            np.full((height, width), np.nan, dtype=prod._type)
        )
        ...
        def chunk_process(args):
            ...
            _p_slope_ = prod.p_slope(
                _sza, _vza, _raa, sigma2=_sigma2, monoview=monoview
            ).values
            ...
            _aot = _R_._interp_aotlut(
                aot_lut.aot_ref.values, _aot_ref, Nwl, Ny, Nx, aot_lut.values
            )
            ...
            for iwl in prod.iwl_swir:
                if monoview:
                    Rf[iwl] = Rcorr[iwl] / (
                        Tdir[iwl] * _Tg_abs[iwl] * _sunglint_eps[iwl] * _p_slope_
                    )
                else:
                    Rf[iwl] = (
                        _sunglint_eps[-1]
        ...
        window_idxs = [
            ...
            for i, j in itertools.product(
                range(0, height, prod.chunk), range(0, width, prod.chunk)
            )
        ]
        ...
        l2_prod = xr.Dataset(
            dict(
                Rrs=(["wl", "y", "x"], np.ctypeslib.as_array(shared_Rrs)),
                BRDFg=(["y", "x"], np.ctypeslib.as_array(shared_Rf)),
                aot550=(["y", "x"], aot_ref_raster.values),
            ),
            coords=dict(wl=prod.raster.wl, x=prod.raster.x, y=prod.raster.y),
        )
        ...
        bitmask = 18
        prod.raster["flags"] = prod.raster.flags + (
            (
                (l2_prod.Rrs.sel(wl=490, method="nearest") < -0.0005)
                | (l2_prod.Rrs.sel(wl=565, method="nearest") < -0.0005)
            )
            << bitmask
        )
        # add name and description
        prod.raster.flags.attrs["flag_descriptions"][bitmask] = (
            "negative Rrs for blue or green bands"
        )
        prod.raster.flags.attrs["flag_names"][bitmask] = "neg_rrs"

        # mask from recipe
        mask = masking_.create_mask(
            prod.raster.flags,
    ...
    def write_output(self):
        ...
        self.l2a.export_to_netcdf(self.ofile, snap_compliant=self.snap_compliant)
