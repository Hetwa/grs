...
class Rasterization:
    ...
    def __init__(self, monoview=False):
        """
    ...
    @staticmethod
    @njit()
    def _interp_Rlut_rayleigh_mono(
        szas, _sza, vzas, _vza, azis, _azi, Nwl, Ny, Nx, lut
    ):
        ...
        for _iy in range(Ny):
            for _ix in range(Nx):
                ...
                for _iwl in range(Nwl):
                    arr_lut[_iwl, _iy, _ix] = (
                        lut[_iwl, isza, ivza, iazi] / mus[_iy, _ix]
                    )
        return arr_lut

    @staticmethod
    @njit()
    def _interp_Rlut_mono(
        szas, _sza, vzas, _vza, azis, _azi, aot_refs, _aot_ref, Nwl, Ny, Nx, lut
    ):
        arr_lut = np.full((Nwl, Ny, Nx), np.nan, dtype=np.float32)
        mus = np.cos(np.radians(_sza))
        for _iy in range(Ny):
            for _ix in range(Nx):
                ...
                for _iwl in range(Nwl):
                    arr_lut[_iwl, _iy, _ix] = (
                        lut[iaot_ref, _iwl, isza, ivza, iazi] / mus[_iy, _ix]
                    )
        return arr_lut

    @staticmethod
    @njit()
    def _interp_Rlut_rayleigh(szas, _sza, vzas, _vza, azis, _azi, Nwl, Ny, Nx, lut):
        arr_lut = np.full((Nwl, Ny, Nx), np.nan, dtype=np.float32)
        mus = np.cos(np.radians(_sza))
        for _iy in range(Ny):
            for _ix in range(Nx):
    ...
    @staticmethod
    @njit()
    def _interp_Tlut(szas, _sza, aot_refs, _aot_ref, Nwl, Ny, Nx, lut):
        arr_lut = np.full((Nwl, Ny, Nx), np.nan, dtype=np.float32)
        for _iy in range(Ny):
            for _ix in range(Nx):
                if np.isnan(_sza[_iy, _ix]):
    ...
    @staticmethod
    @njit()
    def _interp_aotlut(aot_refs, _aot_ref, Nwl, Ny, Nx, lut):
        arr_lut = np.full((Nwl, Ny, Nx), np.nan, dtype=np.float32)
...
class GaseousTransmittance(Gases):
    ...
    def Tgas_background(self):
        ...
        for val in vals:
            ...
            Tg_raster.append(
                xr.DataArray(
                    Tg_int, name="Ttot", coords={"wl": SRF_hr.wl.values}
    ...
    def Tgas(self, gas_name, coef_abs_scat=1):
        """
    ...
    def get_gaseous_optical_thickness(self):
        ...
        ot_air = (
            (
                gas_lut.co
                + self.coef_abs_scat["co2"] * gas_lut.co2
                + self.coef_abs_scat["o2"] * gas_lut.o2
                + self.coef_abs_scat["o4"] * gas_lut.o4
            )
            * self.pressure
            / 1000
        )
    ...
    def get_gaseous_transmittance(
        self, gases=["ch4", "no2", "o3", "h2o"], background=True
    ):
        """
        ...
        for gas in gases:
            if first:
                ...
                Tg_tot = Tg_tot * self.Tgas(gas, self.coef_abs_scat[gas])
    ...
    def water_vapor_correction(
        self, raster_name="coarse_masked_raster", variable="Rtoa_masked"
    ):
        """
        ...
        if self.Twv_raster is None:
            print(
                "xarray of water vapor transmittance is not set, please run get_wv_transmittance_raster(tcwv_raster)"
            )
            return
