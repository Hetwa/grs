...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.debug(error)
        logging.debug(
            "check the symbolic link from "
            + data["auxdata_path"]
            + "to /tmp/grs/.snap/auxdata/dem"
        )
    ...
    try:
        if data["activate_dem"]:
            if os.path.exists("/tmp/grs/.snap/auxdata/dem") and not os.path.islink(
                "/tmp/grs/.snap/auxdata/dem"
            ):
                import shutil
    ...
    outfile = misc.set_ofile(
        file.split("/")[-1], odir=data["output_dir"], level_name="l2grs", suffix=suffix
    )
    ...
    try:
        grs_process.Process().execute(
            ...
            dem=dem,
            aeronet_file=data["aeronet_file"],
            resolution=data["resolution"],
            aot550=data["aot550"],
            angstrom=data["angstrom"],
            unzip=unzip,
            untar=untar,
            startrow=startrow,
            maja_xml=data["maja_xml"],
            waterdetect_file=data["waterdetect_file"],
            waterdetect_only=waterdetect_only,
            memory_safe=data["memory_safe"],
            angleonly=data["angleonly"],
            grs_a=data["grs_a"],
            output=data["output"],
            xblock=data["xblock"],
            yblock=data["yblock"],
